...
description: "LYNX implements six sorting algorithms without STL: HeapSort and ShellSort as generics, plus QuickSort, MergeSort, TimSort, and CountingSort for domain types."
---
...
The comparator overload passes `compare` down into `heapify`, where it replaces the `>` check. The comparator should return `true` if the first argument should be treated as "larger" in the heap — meaning it will ultimately sort to a lower index (i.e., come first in the final array). To sort descending by rating, pass `a.getRating() > b.getRating()`.
